	...
	<body align="center">
		<div class="dk-box">
			...
			<form name="msgForm" id="msgForm" action="<c:url value='  '/>" method="post">
				<c:if test="${id != null}">
					${id}<spring:message code="board.chatting.EgovMessenger" />
					<br>
					<input class="btn btn-primary" type="button" id="connectMsgBtn" name="connectMsgBtn" value="<spring:message code="board.chatting.EgovMessenger0.1"/>" />
					<button class="btn btn-danger"><spring:message code="board.chatting.EgovMessenger0.2" /></button>
				</c:if>
				<c:if test="${id == null}">
					<spring:message code="board.chatting.EgovMessenger1" />
					<br>
