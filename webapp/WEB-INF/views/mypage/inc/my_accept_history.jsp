											...
											<tr>
												...
												<c:choose>
													<c:when test="${i.STATE eq 'wait'}">
														<td><spring:message code="mypage.my_accept_history14" /></td>
														<td>
															<c:choose>
																<c:when test="${nowDays <= boardDays}">
																	<a href="${pageContext.request.contextPath}/mypage/acceptRequest.htm?parti_num=${i.PARTI_NUM}&board_num=${i.BOARD_NUM}&ctn=${i.CATEGORY_NUM}&pc=${i.PARTI_CAPACITY}" class="btn btn-success accept">
																		<spring:message code="mypage.my_accept_history15" />
																	</a>
																	<a href="${pageContext.request.contextPath}/mypage/delete_send_history.htm?user_num=${sessionScope.user_num}&parti_num=${i.PARTI_NUM}&check=2" class="btn btn-danger delete">
																		<spring:message code="mypage.my_accept_history16" />
																	</a>
																</c:when>
																<c:otherwise>
													...
													<c:when test="${i.STATE eq 'refuse'}">
														<td><spring:message code="mypage.my_accept_history18" /></td>
														<td><spring:message code="mypage.my_accept_history19" /></td>
													</c:when>
													<c:when test="${i.STATE eq 'cansel'}">
														<td><spring:message code="mypage.my_accept_history20" /></td>
														<td><spring:message code="mypage.my_accept_history21" /></td>
													</c:when>
													<c:otherwise>
														<td><spring:message code="mypage.my_accept_history22" /></td>
														<td><spring:message code="mypage.my_accept_history23" /></td>
													</c:otherwise>
												</c:choose>
											</tr>
										</c:forEach>
					...
					<div class="pagination-box">
						<ul class="pagination">
							<c:choose>
								...
								<c:otherwise>
									<li class="active">
										<a href="${pageContext.request.contextPath}/mypage/my_page_accept_history.htm?pg=${page-1}&user_num=${sessionScope.user_num}">
							...
							<c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
								<c:choose>
									...
									<c:otherwise>
										<li>
											<a href="${pageContext.request.contextPath}/mypage/my_page_accept_history.htm?pg=${a}&user_num=${sessionScope.user_num}">${a}</a>
										</li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
